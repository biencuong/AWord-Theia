...
lines.push(
    ...
    '  ; Logic chọn CẬP NHẬT/GIỮ như settings.json; sao lưu thành CLAUDE.truoc-cap-nhat.md.',
    ...
    '    Goto xong_claude_md',
    '  hoi_claude_md:',
    '  MessageBox MB_YESNO|MB_ICONQUESTION "Máy này đã có tệp quy tắc làm việc của Claude (CLAUDE.md).$\\r$\\n$\\r$\\nBạn có muốn CẬP NHẬT theo bản mới của AWord không?$\\r$\\n$\\r$\\n• CÓ (khuyến nghị): nhận quy trình đọc PDF scan mới nhanh gấp 3–5 lần và kỹ năng đọc văn bản doc-van-ban-local. Mọi chỉnh sửa cá nhân trong CLAUDE.md sẽ bị thay thế — bản cũ được SAO LƯU thành CLAUDE.truoc-cap-nhat.md.$\\r$\\n$\\r$\\n• KHÔNG: giữ quy tắc hiện tại — AWord vẫn chạy bình thường nhưng Claude tiếp tục đọc PDF scan theo cách cũ (chậm hơn) và không biết kỹ năng đọc văn bản mới." /SD IDNO IDNO xong_claude_md',
    '    CopyFiles /SILENT "$PROFILE\\.claude\\CLAUDE.md" "$PROFILE\\.claude\\CLAUDE.truoc-cap-nhat.md"',
    '    SetOutPath "$PROFILE\\.claude"',
    '    File "/oname=CLAUDE.md" "${PROJECT_DIR}\\..\\CLAUDE.user.md"',
    '  xong_claude_md:',
    '',
