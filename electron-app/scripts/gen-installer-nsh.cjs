...
const lines = [
    ...
    '; 2) Đăng ký menu chuột phải Windows "Mở bằng AWord Pro" cho tệp/thư mục (HKCU,',
...
lines.push(
    ...
    '  ; Thư mục cache ảnh PDF scan — tạo sẵn để additionalDirectories trong settings.json',
    '  ; trỏ tới nơi có thật (Read ảnh không bị hỏi quyền từng trang).',
    '  CreateDirectory "$PROFILE\\.claude\\aword_pdf_cache"',
    ...
    '  CreateShortCut "$SMPROGRAMS\\Kết nối Kho dữ liệu (AWord Pro).lnk" "$INSTDIR\\Ket_Noi_KhoDuLieu.cmd" "" "$INSTDIR\\AWordPro.exe" 0',
    '',
    ...
    '  Delete "$SMPROGRAMS\\Kết nối Kho SGK (AWord Pro).lnk"',
    '  Delete "$INSTDIR\\Ket_Noi_KhoSGK.cmd"',
    '  File "${PROJECT_DIR}\\..\\Ket_Noi_KhoTriThuc.cmd"',
    '  CreateShortCut "$SMPROGRAMS\\Kết nối Kho tri thức AI (AWord Pro).lnk" "$INSTDIR\\Ket_Noi_KhoTriThuc.cmd" "" "$INSTDIR\\AWordPro.exe" 0',
    '  File "${PROJECT_DIR}\\..\\Chuyen_Du_Lieu_AGiaoAn.cmd"',
    '  CreateShortCut "$SMPROGRAMS\\Chuyển dữ liệu AGiaoAn sang AWord Pro.lnk" "$INSTDIR\\Chuyen_Du_Lieu_AGiaoAn.cmd" "" "$INSTDIR\\AWordPro.exe" 0',
    '',
    ...
    '  CreateShortCut "$SMPROGRAMS\\Cài công cụ tài liệu (AWord Pro).lnk" "$INSTDIR\\Cai_Dat_Cong_Cu.cmd" "" "$INSTDIR\\AWordPro.exe" 0',
    '',
    ...
    `  CreateShortCut "$SMPROGRAMS\\Cập nhật Claude (AWord Pro).lnk" "powershell.exe" '-NoProfile -ExecutionPolicy Bypass -File "$INSTDIR\\Cap_Nhat_Claude.ps1" -InstallDir "$INSTDIR"' "$INSTDIR\\AWordPro.exe" 0`,
    ...
    '  WriteRegStr HKCU "Software\\Classes\\*\\shell\\AWordPro" "" "Mở bằng AWord Pro"',
    '  WriteRegStr HKCU "Software\\Classes\\*\\shell\\AWordPro" "Icon" "$INSTDIR\\AWordPro.exe"',
    `  WriteRegStr HKCU "Software\\Classes\\*\\shell\\AWordPro\\command" "" '"$INSTDIR\\AWordPro.exe" "%1"'`,
    '  ; Menu chuột phải: thư mục',
    '  WriteRegStr HKCU "Software\\Classes\\Directory\\shell\\AWordPro" "" "Mở thư mục bằng AWord Pro"',
    '  WriteRegStr HKCU "Software\\Classes\\Directory\\shell\\AWordPro" "Icon" "$INSTDIR\\AWordPro.exe"',
    `  WriteRegStr HKCU "Software\\Classes\\Directory\\shell\\AWordPro\\command" "" '"$INSTDIR\\AWordPro.exe" "%1"'`,
    '  ; Menu chuột phải: nền thư mục đang mở (khoảng trống trong Explorer)',
    '  WriteRegStr HKCU "Software\\Classes\\Directory\\Background\\shell\\AWordPro" "" "Mở thư mục bằng AWord Pro"',
    '  WriteRegStr HKCU "Software\\Classes\\Directory\\Background\\shell\\AWordPro" "Icon" "$INSTDIR\\AWordPro.exe"',
    `  WriteRegStr HKCU "Software\\Classes\\Directory\\Background\\shell\\AWordPro\\command" "" '"$INSTDIR\\AWordPro.exe" "%V"'`,
    '',
    ...
    '  Delete "$SMPROGRAMS\\Kết nối Kho dữ liệu (AWord Pro).lnk"',
    '  Delete "$SMPROGRAMS\\Kết nối Kho tri thức AI (AWord Pro).lnk"',
    '  Delete "$SMPROGRAMS\\Kết nối Kho SGK (AWord Pro).lnk"',
    ...
    '  DeleteRegKey HKCU "Software\\Classes\\Directory\\Background\\shell\\AWordPro"',
    '!macroend',
    '',
    ...
    '  Function StartApp',
    '    ${StdUtils.ExecShellAsUser} $0 "powershell.exe" "open" "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command $\\"Start-Sleep -Seconds 2; Start-Process -FilePath \'$INSTDIR\\AWordPro.exe\'$\\""',
    '  FunctionEnd',
    '  !define MUI_FINISHPAGE_RUN',
    '  !define MUI_FINISHPAGE_RUN_TEXT "Mở AWord Pro (chạy ở nền)"',
